...
```json
{
  "content": "Natural language text to store as memory",
  "category": "namespace-name",
  "metadata": {
    "user_id": "user123",
    "conversation_id": "conv456"
  },
  "graphFilter": "extend"  // optional: "update" | "extend" | "derive"
}
```
...
Semantic search with connected subgraph retrieval.
...
```json
{
  ...
  "namespace": null,       // optional, null for all namespaces
  "relationshipTypes": [   // optional
    "UPDATES", "EXTENDS", "DERIVES", "SIMILAR_TO", "RELATED_TO"
  ],
  "includeSubgraph": true  // optional, default: true
}
```
...
- `maxNodes` (optional, default: 50): Maximum number of nodes
- `similarityThreshold` (optional, default: 0.75): Minimum similarity for relationships
- `sync` (optional, default: false): Sync from Pinecone before fetching

**Example:**
```
GET /get-graph?namespace=user-research&maxNodes=100&similarityThreshold=0.7&sync=true
```
...
#### 5. **POST /get-graph/sync**
Manually sync data from Pinecone to Neo4j.

**Request Body:**
```json
{
  "namespace": null,           // optional, null for all namespaces
  "similarityThreshold": 0.7   // optional, default: 0.7
}
```
...
```json
{
  ...
  "data": {
    ...
    "namespacesProcessed": ["default", "user-research"]
  }
}
```
...
#### 6. **GET /get-graph/namespaces**
Get all available namespaces from Neo4j.
...
```json
{
  "status": "success",
  "data": {
    "namespaces": ["default", "user-research", "team-insights"]
  }
}
```
...
#### 7. **GET /visualize**
Get all memories from Pinecone for basic visualization.

**Query Parameters:**
- `namespace` (optional): Filter by namespace
- `limit` (optional, default: 1000): Maximum number of memories
...
```json
{
  ...
  "data": {
    "memories": [...],
    "total": 250,
    "namespaces": ["default", "user-research"]
  }
}
```
...
#### 8. **POST /get-response**
Generate AI response with context from memories.
...
```javascript
// Using curl
curl -X POST http://localhost:3003/add-memory \
...
```javascript
const searchResult = await fetch('http://localhost:3003/semantic-search', {
  method: 'POST',
...
console.log(data.data.seedMemories);  // Direct matches
console.log(data.data.subgraph.nodes); // Connected memories
```
...
# Run specific tests
npm run test:api          # Test memory creation
npm run test:response     # Test response generation
npm run test:search       # Test semantic search
npm run test:filters      # Test graph filters
```
